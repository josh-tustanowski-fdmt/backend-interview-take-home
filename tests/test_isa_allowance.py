...
class TestIsaAllowanceCalculatorStep1(TestCase):
    ...
    def test_simple_non_flexible_isa_allowance_calculator_with_withdrawal(self) -> None:
        ...
        transactions = [
            Transaction(account=account, amount=Decimal(5_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=account, amount=Decimal(10_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=account, amount=Decimal(-10_000), transaction_date=dt.date(2024, 6, 10)),
        ]
        allowance = calculate_isa_allowance_for_account(account=account, transactions=transactions, tax_year=2024)
        assert allowance.annual_allowance == Decimal(20_000)
        assert allowance.remaining_allowance == Decimal(5_000)
...
class TestIsaAllowanceCalculatorStep3(TestCase):
    def test_multiple_isa_accounts_both_flexible(self) -> None:
        flex_account1 = Account(id=1, client_id=1, account_type=AccountType.Flexible_ISA)
        flex_account2 = Account(id=2, client_id=1, account_type=AccountType.Flexible_ISA)
        transactions = [
            Transaction(account=flex_account1, amount=Decimal(10_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=flex_account2, amount=Decimal(5_000), transaction_date=dt.date(2024, 6, 10)),
        ]
        account1_allowance = calculate_isa_allowance_for_account(account=flex_account1, transactions=transactions, tax_year=2024)
        account2_allowance = calculate_isa_allowance_for_account(account=flex_account2, transactions=transactions, tax_year=2024)
    ...
    def test_multiple_isa_accounts_one_flexible_one_non_flexible(self) -> None:
        non_flex_account = Account(id=1, client_id=1, account_type=AccountType.Flexible_ISA)
        flex_account = Account(id=2, client_id=1, account_type=AccountType.Flexible_ISA)
        transactions = [
            Transaction(account=non_flex_account, amount=Decimal(10_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=non_flex_account, amount=Decimal(-10_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=flex_account, amount=Decimal(5_000), transaction_date=dt.date(2024, 6, 10)),
            Transaction(account=flex_account, amount=Decimal(-3_000), transaction_date=dt.date(2024, 6, 10)),
        ]
        account1_allowance = calculate_isa_allowance_for_account(account=non_flex_account, transactions=transactions, tax_year=2024)
        account2_allowance = calculate_isa_allowance_for_account(account=flex_account, transactions=transactions, tax_year=2024)
...
class TestIsaAllowanceCalculatorStep4(TestCase):
    def test_lifetime_isa_contributions(self) -> None:
        lisa_account = Account(id=1, client_id=1, account_type=AccountType.Flexible_Lifetime_ISA)
        transactions = [
            Transaction(account=lisa_account, amount=Decimal(2_000), transaction_date=dt.date(2024, 5, 10)),
            Transaction(account=lisa_account, amount=Decimal(-1_000), transaction_date=dt.date(2024, 5, 10)),

        ]
        allowance = calculate_isa_allowance_for_account(account=lisa_account, transactions=transactions, tax_year=2024)
    ...
    def test_lifetime_isa_contributions_overcontribution(self) -> None:
        lisa_account = Account(id=1, client_id=1, account_type=AccountType.Flexible_Lifetime_ISA)
        transactions = [
            Transaction(account=lisa_account, amount=Decimal(5_000), transaction_date=dt.date(2024, 5, 10)),
        ]
        allowance = calculate_isa_allowance_for_account(account=lisa_account, transactions=transactions, tax_year=2024)
...
class TestIsaAllowanceCalculatorStep5(TestCase):
    def test_combined_isa_and_lisa_with_large_isa_contributions(self) -> None:
        ...
        * Example below:
        ...
        """
        isa_account = Account(id=1, client_id=1, account_type=AccountType.Flexible_ISA)
        lisa_account = Account(id=2, client_id=1, account_type=AccountType.Flexible_Lifetime_ISA)
        transactions = [
            Transaction(account=isa_account, amount=Decimal(17_000),  transaction_date=dt.date(2024, 5, 10))
        ]
        
        lisa_allowance = calculate_isa_allowance_for_account(account=lisa_account, transactions=transactions, tax_year=2024)
        assert lisa_allowance.remaining_allowance == Decimal(3_000)
        assert lisa_allowance.annual_allowance == Decimal(3_000)
        
        isa_allowance = calculate_isa_allowance_for_account(account=isa_account, transactions=transactions, tax_year=2024)
    ...
    def test_combined_isa_and_lisa_with_maxed_lisa_contribution(self) -> None:
        ...
        * Example below:
        ...
        lisa_account = Account(id=2, client_id=1, account_type=AccountType.Flexible_Lifetime_ISA)
        transactions = [
            Transaction(account=lisa_account, amount=Decimal(4_000),  transaction_date=dt.date(2024, 5, 10))
        ]
...
class TestIsaAllowanceCalculatorStep6(TestCase):
    def test_adjust_isa_annual_allowance_from_inheritance(self) -> None:
        ...
        * APS Transfer into an ISA of 40k
        ...
        isa_account = Account(id=1, client_id=1, account_type=AccountType.Flexible_ISA)
        transactions = [
            Transaction(account=isa_account, amount=Decimal(4_000), transaction_date=dt.date(2024, 5, 10))
        ]
